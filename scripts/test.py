...
def prep_from_mlflow_name(run_name: str=None, run_id: str=None):
    ...
    # retrieve mlflow run
    if run_id is None:
        run_ids = mlflow.search_runs(filter_string=f"attributes.run_name='{run_name}'", search_all_experiments=True)['run_id']
        if len(run_ids) == 0:
            return None, None, None, None, None, None
...
def main(
    ...
    Args:
        run_name: Name of the MLflow run to evaluate (something like joyful-seal-1243). Can also be experiment name, like CFLP-toy
    ...
    if model is None:
        mlrun = get_most_recent_run_from_experiment_name(run_name, solver)
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    main(args.run_name, args.cores, args.epsilon, args.alpha, solver=args.solver, device=args.device)